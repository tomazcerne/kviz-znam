...
<body>
       <div id="naslov"><?php echo $naslov_kviza ?> - top 10</div>
       <div id="vsebina">
            <?php
                require("../podatki/podatki.php");
                $baza = new mysqli($p[0], $p[1], $p[2], $p[3]);
                if($baza->connect_error){
                    ...
                    $pridobi_podatke = $baza->prepare("SELECT uporabnisko_ime, st_pravilnih, st_vprasanj, datum_rezultata
                        FROM uporabniki INNER JOIN rezultati ON uporabniki.id_uporabnika = rezultati.id_uporabnika AND rezultati.id_kviza = ? ORDER BY 1.0*st_pravilnih/st_vprasanj DESC, datum_rezultata ASC LIMIT 10");
                    $pridobi_podatke->bind_param("i", $_GET["id_kviza"]);
                    if($pridobi_podatke->execute()){
                        $rezultat = $pridobi_podatke->get_result();
                        if($rezultat->num_rows == 0){
       ...
       <div id="noga">
            <a id="nazaj" href="kvizi.php">nazaj</a>
       </div>
